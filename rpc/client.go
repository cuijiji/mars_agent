...
type Client interface {
	...
	Wait(id string) (*pb2.Status, error)
	...
	// called.
	GetStatus(id string) (*pb2.Status, error)
...
func (c *client) Open(host, port string) error {
	...
	c.conn = conn
	c.agent = pb2.NewRCEAgentClient(conn)
	c.host = host
	c.port = port
	return nil
}
...
func (c *client) Wait(id string) (*pb2.Status, error) {
	return c.agent.Wait(context.TODO(), &pb2.ID{ID: id})
}

func (c *client) GetStatus(id string) (*pb2.Status, error) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	return c.agent.GetStatus(ctx, &pb2.ID{ID: id})
}

func (c *client) Stop(id string) error {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	_, err := c.agent.Stop(ctx, &pb2.ID{ID: id})
	return err
}
...
func (c *client) Running() ([]string, error) {
	...
	stream, err := c.agent.Running(ctx, &pb2.Empty{})
	if err != nil {
		return nil, err
	}
